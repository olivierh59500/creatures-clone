"github.com/hajimehoshi/ebiten/v2/vector"// Layout returns the screen dimensions
func (g *Game) Layout(outsideWidth, outsideHeight int) (int, int) {
	return g.config.ScreenWidth, g.config.ScreenHeight
	// Number keys - place different food types
	foodTypes := []objects.FoodType{
		objects.FoodApple,
		objects.FoodCarrot,
		objects.FoodHoney,
		objects.FoodBerry,
	...
	for i, foodType := range foodTypes {
		...
		if inpututil.IsKeyJustPressed(key) {
			food := objects.NewFood(worldX, worldY, foodType)
